...
\newcommand{\atnil}{\fclcode{@nil}\xspace}
...
\subsection{\atnil value}

The literal \atnil serves as a placeholder value, distinct from all
...
sequence with an additional element\footnote{%
  ...
  n-1$.  Extending a sequence will implicitly insert \atnil values, if
...
\begin{table}
  \caption{List of keys associated with above document.  All keys that begin with the name \fclcode{t} are fully qualified keys.}
  \begin{center}
    \begin{tabular}{ll} \hline\hline
...
contained in the corresponding named file.\footnote{%
  If it is set, the environment variable \fclcode{FHICL_FILE_PATH} is
...
placed in files that are \fclcode{#include}d.  This ensures that users
...
a.b1 : @erase
\end{fcllisting}
the name \fclcode{b1} is removed, and the table \fclcode{a} is
empty.
...
\item [\textbf{Error}] For a value bound using \protectError, a
  ...
  which a diagnostic message is provided by the binding language.
\end{description}

\subsection{Protection inheritance}

During assignment, a protection level is inherited from an enclosing
name if the nested name has no specified protection.  It is an
